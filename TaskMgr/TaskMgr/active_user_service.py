...
class ActiveUserValidationTests(unittest.TestCase):
    ...
    # Проверяет разные варианты осмысленности описания.
    # @returns None.
    # @throws AssertionError если проверка описания ошибочно примет или отклонит значение.
    # @note описание должно быть похоже на текст, а не на набор цифр или символов.
    def test_meaningful_description_validation_cases(self):
        cases = [
            ("Пользователь активно участвует в группе", True),
            ("Студент 2 курса активно пишет комментарии", True),
            ("1234567890", False),
            ("!!!!!!!!!!", False),
            ("аааааааааа", False),
            ("123456789012345 актив", False),
        ]

        for value, expected in cases:
            with self.subTest(value=value):
                self.assertEqual(is_meaningful_description(value), expected)
    ...
    # @note тест проверяет длину описания и подключение is_meaningful_description.
